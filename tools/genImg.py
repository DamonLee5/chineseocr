...
def generateImg(ind):
    global imgDir
    #captcha=ImageCaptcha(fonts=['./fonts/msyhbd.ttf'])
    img = Image.new('RGB', (120, 32), color = 'white')
    theChars=gen_rand()
    #data=captcha.generate(theChars)
    img_name= '{:08d}'.format(ind)+'.jpg'
    img_path=imgDir+'/'+img_name
    d = ImageDraw.Draw(img)
    # fc-list :lang=zh (查看Linux中文字体存放位置)
    font = ImageFont.truetype('/usr/share/fonts/opentype/noto/NotoSerifCJK-Black.ttc',24)
    d.text((0,0), theChars, fill=(0,0,0),font=font)
    #captcha.write(theChars,img_path)
    text_name='{:08d}'.format(ind)+'.txt'
    text_path=imgDir+'/'+text_name
    img.save(img_path)
    fout=open(text_path,'w')
    fout.write(theChars)
    fout.close()
    print(img_path)
